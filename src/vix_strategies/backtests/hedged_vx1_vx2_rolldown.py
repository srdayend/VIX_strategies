...
def prepare_vx1_vx2_frame() -> pd.DataFrame:
    ...
    df = df.rename(columns={"Trade Date": "Date", "M1 Settle": "VX1", "M2 Settle": "VX2", "M1 Month": "VX1_Month", "M2 Month": "VX2_Month"})
...
def run_hedged_rolldown(config: HedgedRollDownConfig = HedgedRollDownConfig()) -> pd.DataFrame:
    ...
    data["roll_day"] = (data["VX1_Month"].ne(data["VX1_Month"].shift(1)) | data["VX2_Month"].ne(data["VX2_Month"].shift(1))).fillna(False)
    ...
    for i in range(len(data)):
        ...
        if not position:
            enter_now = data.loc[i, "front_slope"] > config.entry_slope and data.loc[i, "VX1"] < config.vx1_cap and (not config.avoid_roll or not data.loc[i, "roll_day"])
            if enter_now:
                position = True
