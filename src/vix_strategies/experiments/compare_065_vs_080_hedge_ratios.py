...
def build_comparison() -> pd.DataFrame:
    ...
    merged["regime_lag"] = [_combined_regime(vix, slope, basis) for vix, slope, basis in zip(merged["VIX_lag"], merged["front_slope_lag"], merged["front_basis_lag"])]
    merged["basis_bucket_lag"] = merged["front_basis_lag"].map(_bucket_basis)
    return merged


def main() -> None:
    output_dir = Path("reports/generated/compare_065_vs_080_hedge_ratios")
